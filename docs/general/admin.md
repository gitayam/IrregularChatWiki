---
title: "Admin"
---

# Admin

### Quick Admin Links
For Admin, See the [Mod Portal](https://mod.irregular.chat) to create users and more


- *''[Chat Message Prompts](/ai-ml/ai-prompting)

- [Links](/general/links) with all the chats

- [Page](/community/welcome)

- [IrregularChat Forum](https://forum.irregularchat.com)


Add ?all to the URL to temporarily use it and bypass the SSO. # Group Admins


Thanks for being an admin. You have your day job, and this is not intended to take much of your time.

The goal is to provide additional resources to those interested in expanding their proficiency. The Wiki and the GitLab can help users greatly in that regard, as so much information gets put out and may need help finding its way to the right people.


## Copy and Paste For Situation

### Individual Requests to Join Chat
As an admin, you help add and confirm people are coming from the community. Confirming someone who requests to join is as easy as using the following:

### Moderation Messages

### Encourage NIPR/SIPR/JWICS for Knowledge Management
```
Are you on the NIPR or higher Milsuite or Chatsurfer?

🍀
Direct NIPR Link Chatsurfer: https://url.irregular.chat/chatsurfer
Direct NIPR Link Milsuite: https://url.irregular.chat/msuite
👹
Direct SIPR Link Chatsurfer: (access via SIPR network)

or search &quot;irregular chat&quot; on chatsurfer NIPR SIPR or higher

Perfect for sharing on the appropriate level while still breaking down silos and being found by others working on these same topics
```

- **[To Top](/general/admin)**


### Send Off Topic
```
🌟Community Reminder🌟

Hey everyone! Our group thrives on technical discussions, sharing knowledge, and building cool projects together.

We understand everyone likes a good laugh or a casual chat now and then beyond the room's topic, which is why we've got a special place just for that!

If you're itching for off-topic banter or to share the latest meme, head over to our off-topic chat: https://url.irregular.chat/otg.

It's the perfect spot for relaxing and diverging from a specific topic of chatter.

If you're not interested in the technical aspects, no worries! We want everyone to find their best fit, and there are no hard feelings if this isn't your place.

Let's keep our main chat focused on tech to ensure it remains a valuable resource for everyone.

Thanks for being such an awesome part of our journey. Let's keep pushing the boundaries of what we can achieve together! 🚀
```

- **[To Top](/general/admin)**

### Bonafides
```
You've requested to join the Irregular Chat.

Bonafides: Everyone in the chat has been invited by another who they personally know. So that we can add you to the right groups, we need to know:
## Who you are
2. Where you are from
3. Who invited you

As you send your intro, feel free to ask any questions about the community
```

- **[To Top](/general/admin)**

### Safety Number Change
```
It shows that your safety number has changed. For the privacy and security of all, we ask that you provide one of the following:
- Your details. Name, organization, and who added you
- Your IrregularChat Element Messenger username that we can verify

If you don't reply to this message in 24 hours, your account will be removed from the chats, and you can request to join again when you are ready.
```

- **[To Top](/general/admin)**

### Removal due to not verifying a safety number
```
NAME is being removed for not verifying themselves after their safety number changed. This is done to maintain the integrity of the community. This could mean the number was assigned to a different person or their SIM was put into a different device.

They are welcome to request to join anytime but will need to be verified by knowing someone in the community and providing their name and organization
```

- **[To Top](/general/admin)**


### Diagram of Irregular Wiki Concept

- Current Wiki Flowchart (see site navigation)


